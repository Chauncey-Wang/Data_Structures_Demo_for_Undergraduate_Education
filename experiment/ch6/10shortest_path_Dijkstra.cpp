...
typedef struct{ 
	VerTexType vexs[6]={'0','1','2','3','4','5'};            		//顶点表 
	ArcType arcs[6][6]={
	...
	int vexnum=6,arcnum=8;                		//图的当前点数和边数 
}AMGraph;
...
void ShortestPath_DIJ(AMGraph G, int v0){ 
	...
	for(i = 1;i < n; ++i){									//对其余n-1个顶点，依次进行计算 
		...
		S[v]=true;                   						//将v加入S 
		for(w = 0;w < n; ++w)           					//更新从v0出发到集合V-S上所有顶点的最短路径长度  判断与v有路径的顶点中，如果有路径更短，则更新
			if(!S[w] && (D[v] + G.arcs[v][w] < D[w])){      //原点经过v到w的长度，小于原点直接到w的长度
